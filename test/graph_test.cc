...
int main() {
	...
	while( true ) {
		...
		for(std::vector<graph_node>::iterator it=tree.begin(); it != tree.end(); ++it) {
			cout << "id: " << (*it)._identity << endl ;
			cout << "num: " << (*it)._num_neighbors << endl ;
			cout << "neighbors: " ;
			for(int i=0; i<(*it)._num_neighbors; ++i) {
				cout << (*it)._neighborhood[i] << " " ;
			}
			cout << endl << "weights: " ;
			for(int i=0; i<(*it)._num_neighbors; ++i) {
				cout << (*it)._distance[i] << " " ;
...
//	graph_node tree[] = { _tree[0], _tree[1], _tree[2], _tree[3], _tree[4], _tree[5], _tree[6], _tree[7]
//		_tree[8], _tree[9], _tree[10], _tree[11], _tree[12], _tree[13], _tree[14], _tree[15], _tree[16], _tree[17],
//		_tree[18], _tree[19] } ;
//	
